...
class Duo4SRec(torch.nn.Module):
    ...
    def cal_loss(self, su_batch, next_req_bacth, su_batch_operat_list, batch_idx):
        ...
        for i in range(batch_size):
            if not torch.any(torch.as_tensor(su_batch_operat_list[0][i]).isnan()):
                su_neg = torch.cat((su_neg, su_batch_operat_list[0][i]), dim = 0);
        if su_batch_operat_list[0][batch_idx] is torch.nan or su_neg.shape[0] == 0:
            return loss_rec + self.lambda_cls * loss_cls;
        #hu_rtrl:(1, d)
        ...
        return loss_rec + self.lambda_sl * loss_cl + self.lambda_cls * loss_cls;
