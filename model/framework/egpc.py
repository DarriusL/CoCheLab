...
class EGPC(torch.nn.Module):
    '''Explanation Guided Proactive Content Caching via Contrastive Learning
    ...
    def _impt_score_step(self, su):
        ...
        hu = self.encoder(su);
        #hreq:(req_len, d)
        hreq = self.encoder(su.reshape(-1, 1));
        #s:(req_len)
        s = torch.matmul(hreq, hu.transpose(0, 1)).squeeze(-1);
        return s;
